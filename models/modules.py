...
class ProgressiveKernelApply(nn.Module):
    ...
    def forward(self, data, kernels, sum_r, sum_w, max_w):
        ...
        Args:
            data(torch.Tensor[bs, chans, h, w]): data to filter with the
                `kernels`.
            kernels(torch.Tensor[bs, k*k, h, w]): gather/splat kernels to apply to
                `data`.
            sum_r(None or torch.Tensor[bs, chans, h, w]): running sum of past
                contributions, expects None for the first call
                (initialization).
            sum_w(None or torch.Tensor[bs, 1, h, w]): running sum of weights,
                expects None for the first call (initialization).
            max_w(None or torch.Tensor[bs, 1, h, w]): running maximum of weights,
                expects None for the first call (initialization).

        Returns:
            sum_r(torch.Tensor[bs, chans, h, w]): updated running sum of past
                contributions
            sum_w(torch.Tensor[bs, 1, h, w]): updated running sum of weights
            max_w(torch.Tensor[bs, 1, h, w]): updated running maximum of weights.
        """
        ...
        if sum_r is None:  # first time we update
            ...
            # sum_r, sum_w = funcs.kernel_weighting(data.contiguous(), kernels.contiguous())
            ...
            new_max = torch.max(kmax, max_w)

            # max_w is the old_max at this point
            scaler = torch.exp(max_w - new_max)
